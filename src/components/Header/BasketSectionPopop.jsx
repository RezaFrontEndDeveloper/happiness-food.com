...
function BasketSectionPopop({ setToggleBasket }) {
  ...
  return (
    <div
      ref={popopRef}
      className="flex  fixed sm:p-10 top-0 right-0 h-screen gap-4  sm:w-1/3 bg-white shadow-xl flex-col">
      <div>
        <IoCloseOutline
          size={30}
      ...
      <div className="w-full flex gap-4 flex-col ">
        {basket.map((item) => (
          <div
            key={item?.id}
            className=" flex justify-between items-center border border-gray-200 p-2 w-full">
            <h2>{item?.title}</h2>
            <p>{item?.price}</p>
            ...
            <IoCloseOutline
              ...
              size={20}
              color="red"
              onClick={() => handleDeleteItemAtBasket(item.id)}
            />
          </div>
        ))}
      </div>
